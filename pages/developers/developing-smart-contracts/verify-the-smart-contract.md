...
3. Navigate to the **Contract** tab and click **Verify & Publish**.
   {% /tab %}
   {% tab label="Testnet" %}
4. Open the **XRPL EVM Testnet Explorer**: [https://explorer.testnet.xrplevm.org](https://explorer.testnet.xrplevm.org).
5. Locate your deployed contract by searching for its **contract address** in the search bar.
6. Navigate to the **Contract** tab and click **Verify & Publish**.
   {% /tab %}
   {% /tabs %}
